...
import { validataSend, validateCodeCheck } from '../../request/userRequest'
...
const MailRegister = ({ setStep,registerInfo,setRegisterInfo}) => {
  ...
  const gotoNext = () => {
    ...
    if (mailErrMsg.show === true || validateErrMsg.show === true) {
      return
    }
    validateCodeCheck({ email: mailBox, validateCode: validateCode }).then((res) => {
      if (res.data.flag === 0) {
        //验证码错误或者是验证码过期
        dispatch({ type: snackBarActionType.ACTION_OPEN, payload: { open: true, message: res.data.message } })
        return;
      }
      //切换到密码设置页面,并且将值返回给主页面
      setStep(1)
      setRegisterInfo({...registerInfo,mailBox:mailBox})
    })
  }
